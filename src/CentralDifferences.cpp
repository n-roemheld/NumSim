...
double CentralDifferences::computeDuvDx (int i, int j) const
{
	...
	return ((u_(i+1,j+1)+u_(i+1,j))*(v_(i,j)+v_(i+1,j))-(u_(i,j+1)+u_(i,j))*(v_(i-1,j)+v_(i,j)))/(4*meshWidth_[0]);
}
...
double CentralDifferences::computeDuvDy (int i, int j) const
{
	...
	return ((v_(i+1,j+1)+v_(i,j+1))*(u_(i,j+1)+u_(i,j))-(v_(i+1,j)+v_(i,j))*(u_(i,j)+u_(i,j-1)))/(4*meshWidth_[1]);
}
