...
void OutputWriterText::writeFile(double currentTime)
{
  ...
  // write u values
  for (int j = discretization_->u().size()[1]-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j << "|";
    for (int i = 0; i < discretization_->u().size()[1];  i++)
    {
  ...
  // write v values
  for (int j = discretization_->v().size()[1]-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j << "|";
    for (int i = 0 ; i < discretization_->v().size()[0] ; i++)
    {
  ...
  file << "p (" << discretization_->p().size()[0] << "x" << discretization_->p().size()[1] << "): " << std::endl
    << std::string(fieldWidth, ' ') << "|";
  for (int i = 0; i < discretization_->p().size()[0]; i++)
  {
    file << std::setw(fieldWidth) << i;
  }
  ...
  // write p values
  for (int j = discretization_->p().size()[1]-1; j >= 0; j--)
  {
    file << std::setw(fieldWidth) << j << "|";
    for (int i = 0; i < discretization_->p().size()[0]; i++)
    {
