...
// Domain parameters
constexpr size_t nx = 20; //< Number of grid cells in x-direction //+ nCells, with ghost cells?
constexpr size_t ny = 20; //< Number of grid cells in y-direction
constexpr double lx = 1.; //< Domain size in x-direction //+ physical size
...
constexpr double thermalDiff = 1. / 224.36; //< Thermal diffusivity //+ = 1/(Re*Pr), add to settings?
...
constexpr double targetTOnInterface = 1.; //+ replace with data from precice


static const std::string solverName = "SolidSolverDummy"; //+ handled in settings
static const std::string meshName = "SolidSolverDummyMesh"; //+ handled in settings
...
//+ nx: number of Interface points
//+ computes (x,y) coordinates of all interface points (hard-coded geometry!)
...
//+ HeatFlux: pass to fluid solver as boundary conditions
// q = - kappa * dT/dy
                        ...
                        std::array<double, nx>& heatFlux )
{
  ...
  const double normalVectorY = -1; //< Normalvector points downwards //+ dynamic adaptation to gemoetry required!
  // Compute heat fluxes
  for (size_t i = 0; i < nx; ++i) {
...
//+ setting boundary temperature in solid solver (here)
...
//+ Replace with proper Temperature solver
...
// Helper function to create and initialize an array //+ useful for us?
...
{
  std::string preciceConfigFileName = "precice-config.xml"; //+ read from input file -> settings
  if ( argc == 2 ) //+ needed?
  {
...
//  std::fill( temperatureInBC.begin(), temperatureInBC.end(), 0.0 );
  ...
  double precice_dt = interface.initialize(); //+ initialize precice communication and get initial maximum precice time step
