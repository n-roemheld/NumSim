...
void OutputWriterParaview::writeFile(double currentTime)
{
  ...
  for (int j = 0; j < nCells[1]+1; j++)
  {
    for (int i = 0; i < nCells[0]+1; i++, index++)
    {
      ...
      arrayPressure->SetValue(index, discretization_->p().interpolateAt(x,y,3));
    }
  }
  ...
  for (int j = 0; j < nCells[1]+1; j++)
  {
    for (int i = 0; i < nCells[0]+1; i++, index++)
    {
      ...
      arrayTemperature->SetValue(index, discretization_->T().interpolateAt(x,y,3));
    }
  }
  ...
  for (int j = 0; j < nCells[1]+1; j++)
  {
    ...
    for (int i = 0; i < nCells[0]+1; i++, index++)
    {
      ...
      velocityVector[0] = discretization_->u().interpolateAt(x,y,1);
      velocityVector[1] = discretization_->v().interpolateAt(x,y,2);
