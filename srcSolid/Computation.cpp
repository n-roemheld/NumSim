...
void Computation::initialize (int argc, char *argv[])
{
	...
	{
		discretization_ = std::make_shared<DonorCell>(settings_->nCells, meshWidth_, settings_->geometryPVString_, settings_->geometryPVOrientation_, settings_->geometryPV1_, settings_->geometryPV2_, settings_->geometryTString_, settings_->geometryT1_, settings_, settings_->alpha, settings_->gamma);
	}
	else
	{
...
void Computation::runSimulation ()
{
	...
	// while (discretization_->adapter.isCouplingOngoing()) // implicit
	{
		...
		// compute T
		computeTemperature(readData); // Reihenfolge?
...
void Computation::computeTemperature (std::vector<double> readData)
{
	temperatureSolver_->solve(dt_, settings_->heatDiffusivity, readData);
};
